angular.module('starter.services')
  ...
  .service('AuthService', ['$q', '$http', function($q, $http) {
    ...
    function useCredentials(phoneNumber) {
      return $http.get('http://192.168.1.4:3000/users', {params:{'phoneNumber':phoneNumber}}).then(function(user) {
        if(angular.isDefined(user.data[0]) && user.data[0] != null) {
          return user.data[0];
        } else {
          return $http.post('http://192.168.1.4:3000/users', {'phoneNumber' : phoneNumber, 'name': phoneNumber}).then(function(user) {
